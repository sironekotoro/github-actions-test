#!/usr/bin/env bash
# Codex CLI agent adapter. CLI: `codex`.
#
# Codex supports two authentication modes:
#   1. OpenAI API key (profile=openai-api) or broker capability
#      (profile=openai-broker). The external credential contract uses
#      OPENAI_API_KEY, while pinned `codex exec` 0.147.0 consumes ephemeral
#      headless API auth from CODEX_API_KEY.
#   2. ChatGPT subscription (profile=chatgpt-subscription) via host-local auth
#
# Subscription mode is represented by the profile matrix but is rejected before
# execution until trusted-host identity and per-job credential cleanup exist.
set -uo pipefail

_ADAPTER_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$_ADAPTER_DIR/../lib/common.sh"
source "$_ADAPTER_DIR/../lib/credentials.sh"

AGENT_NAME="codex"

agent_check_available() {
  command -v codex >/dev/null 2>&1
}

agent_get_version() {
  # Version discovery needs no provider credential.
  agent_exec_clean "" "" -- codex --version 2>&1 | head -1
}

agent_run() {
  local model="$1" prompt="$2" logfile="$3" max_runtime="$4"
  local credential_var="$5" credential_value="$6"

  # Keep the external profile/secret contract as OPENAI_API_KEY, but translate
  # only at the Codex process boundary. In broker mode the value is an opaque
  # short-lived capability rather than an OpenAI credential. `codex exec`
  # 0.147.0 sends CODEX_API_KEY as its bearer credential.
  [ "$credential_var" = "OPENAI_API_KEY" ] \
    || fail_with "$CAT_AGENT_AUTH" "Codex OpenAI profile resolved an unexpected credential variable"
  [ -n "$model" ] \
    || fail_with "$CAT_AGENT_START" "Codex requires an explicit requested model"

  # B2a proved that omitting -m causes Codex 0.147.0 to silently use its own
  # default model, which breaks broker exact-model binding. Always bind the CLI
  # invocation to the trusted normalized requested model.
  #
  # The trusted Agent Dispatch Docker container is the actual capability
  # boundary: non-root, read-only rootfs, no host HOME/.git/GitHub credentials
  # or Docker socket, provider-allowlisted egress, read-only /baseline, and only
  # the disposable /workspace bind mounted writable. Codex's nested Linux
  # sandbox can conflict with that external sandbox and silently prevent edits.
  # Keep --skip-git-repo-check because the disposable workspace has no .git.
  agent_run_clean "CODEX_API_KEY" "$credential_value" "$max_runtime" "$logfile" -- \
    codex exec -m "$model" --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check "$prompt"
}

is_auth_agent_error() {
  local logfile="$1"
  grep -qiE '401 Unauthorized|Missing bearer or basic authentication|invalid[_ -]?api[_ -]?key|incorrect API key|authentication (failed|required)' "$logfile" 2>/dev/null
}

is_transient_agent_error() {
  local logfile="$1"
  grep -qiE '429|rate.?limit|ECONNRESET|ETIMEDOUT|fetch failed|5[0-9]{2}|server error|temporarily' "$logfile" 2>/dev/null
}
