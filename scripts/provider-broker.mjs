...
const ANTHROPIC_LIVE_FORWARDING_ENABLED = String(process.env.ANTHROPIC_LIVE_FORWARDING_ENABLED || 'false').trim().toLowerCase() === 'true';
...
function isLocalHttpUrl(rawUrl) {
  try {
    ...
    return url.protocol === 'http:' && (url.hostname === '127.0.0.1' || url.hostname === 'localhost' || url.hostname === 'mock');
  } catch {
    return false;
  }
}
...
function validateConfiguration() {
  if (!['openrouter', 'openai', 'anthropic'].includes(PROVIDER)) {
    throw new Error('BROKER_PROVIDER must be openrouter, openai, or anthropic');
  }
  ...
  if (
    PROVIDER === 'anthropic' &&
    !isLocalHttpUrl(ANTHROPIC_PROVIDER_API_URL) &&
    !ANTHROPIC_LIVE_FORWARDING_ENABLED
  ) {
    throw new Error('Anthropic live forwarding is disabled until B3c budget enforcement is enabled');
  }
  ...
  if (BODY_LIMIT_RAW) {
    ...
    // B3a observed ~83 KiB for a tiny Claude Code prompt because the CLI sends
    // its complete tool/system surface. Keep a Claude-specific ceiling with
    // headroom without exposing an unbounded proxy.
...
async function provisionOpenAiCredential() {
  let result = await openAiAdminFetch('POST', '/organization/projects', { name: safeTaskName('agent-broker') });
  if (!isSuccessStatus(result.status) || !result.body?.id) throw new Error(`OpenAI project creation failed: HTTP ${result.status}`);
  openAiProjectId = result.body.id;

  result = await openAiAdminFetch('POST', `/organization/projects/${openAiProjectId}/spend_limit`, {
    currency: 'USD', interval: 'month', threshold_amount: openAiSpendLimitCents,
  });
  if (!isSuccessStatus(result.status) || result.body?.currency !== 'USD' || result.body?.interval !== 'month' || Number(result.body?.threshold_amount) !== openAiSpendLimitCents || result.body?.enforcement?.status !== 'enforcing') {
  ...
  result = await openAiAdminFetch('POST', `/organization/projects/${openAiProjectId}/model_permissions`, {
    mode: 'allow_list', model_ids: [CAPABILITY_MODEL],
  });
  if (!isSuccessStatus(result.status) || result.body?.mode !== 'allow_list' || !Array.isArray(result.body?.model_ids) || result.body.model_ids.length !== 1 || result.body.model_ids[0] !== CAPABILITY_MODEL) {
  ...
  const disabledHostedTools = {
    file_search: { enabled: false }, web_search: { enabled: false }, image_generation: { enabled: false },
    mcp: { enabled: false }, code_interpreter: { enabled: false },
  };
  result = await openAiAdminFetch('POST', `/organization/projects/${openAiProjectId}/hosted_tool_permissions`, disabledHostedTools);
  const hostedToolsDisabled = Object.keys(disabledHostedTools).every((name) => result.body?.[name]?.enabled === false);
  if (!isSuccessStatus(result.status) || !hostedToolsDisabled) throw new Error(`OpenAI hosted-tool deny policy was not confirmed: HTTP ${result.status}`);
  log('info', 'OpenAI hosted tools disabled');

  result = await openAiAdminFetch('POST', `/organization/projects/${openAiProjectId}/service_accounts`, { name: safeTaskName('agent-job') });
  if (!isSuccessStatus(result.status) || !result.body?.id || result.body?.role !== 'member' || !result.body?.api_key?.id || !result.body?.api_key?.value) {
...
async function provisionProviderCredential() {
  if (PROVIDER === 'openai') return provisionOpenAiCredential();
  if (PROVIDER === 'anthropic') {
    // Anthropic does not currently provide the same per-job disposable-key /
    // provider-side hard-cap contract used for OpenRouter and OpenAI here.
    // B3b therefore keeps the long-lived API key inside the trusted broker and
    // refuses non-local live forwarding by default. B3c must add a conservative
    // per-job budget guard before production enablement.
    upstreamCredential = ANTHROPIC_API_KEY;
    log('info', 'Anthropic broker credential loaded into trusted process');
    return;
  }
  return provisionOpenRouterCredential();
}
...
async function cleanupOpenRouterCredential() {
  ...
  try {
    ...
    if (status === 200 || status === 204) { log('info', 'temporary key deleted'); return; }
    log('warn', 'delete failed; expiry fallback');
  } catch { log('warn', 'delete failed; expiry fallback'); }
}
...
async function cleanupOpenAiCredential() {
  ...
  if (openAiProjectId && openAiServiceAccountId) {
    try {
      const result = await openAiAdminFetch('DELETE', `/organization/projects/${openAiProjectId}/service_accounts/${openAiServiceAccountId}`);
    ...
    } catch { log('warn', 'OpenAI service-account delete failed; project hard limit remains'); }
  }
  if (openAiProjectId) {
    try {
      const result = await openAiAdminFetch('POST', `/organization/projects/${openAiProjectId}/archive`, null);
      if (isSuccessStatus(result.status) && result.body?.status === 'archived') log('info', 'ephemeral OpenAI project archived');
      else log('warn', 'OpenAI project archive failed; hard limit remains');
    } catch { log('warn', 'OpenAI project archive failed; hard limit remains'); }
  }
}
...
async function cleanupProviderCredential() {
  ...
  else if (PROVIDER === 'openrouter') await cleanupOpenRouterCredential();
  upstreamCredential = null;
}
...
function presentedCapability(req) {
  if (PROVIDER === 'anthropic') return String(req.headers['x-api-key'] || '');
  return parseBearer(req.headers.authorization) || '';
}

function requestUrl(req) {
  try { return new URL(req.url || '/', 'http://broker.local'); } catch { return null; }
}

function isAllowedPathAndMethod(method, pathname) {
  if (PROVIDER === 'openrouter') return method === 'POST' && pathname === '/api/v1/chat/completions';
  if (PROVIDER === 'anthropic') return method === 'POST' && pathname === '/v1/messages';
...
function buildUpstreamHeaders(reqHeaders) {
  const allowlist = new Set([
    'content-type', 'accept', 'accept-encoding', 'user-agent', 'originator', 'version',
    'anthropic-version', 'anthropic-beta',
  ]);
  const blocked = new Set([
    'authorization', 'proxy-authorization', 'proxy-authenticate', 'www-authenticate',
    'x-forwarded-for', 'x-forwarded-proto', 'x-forwarded-host', 'forwarded', 'via', 'x-real-ip',
    'cookie', 'set-cookie', 'x-api-key', 'api-key', 'openai-organization', 'openai-project',
  ]);
  const result = {};
...
function providerBaseUrl() {
  if (PROVIDER === 'openai') return OPENAI_PROVIDER_API_URL;
  if (PROVIDER === 'anthropic') return ANTHROPIC_PROVIDER_API_URL;
  return OPENROUTER_PROVIDER_API_URL;
}

function validateOpenAiRequestShape(parsedBody, res) {
  if (parsedBody?.stream !== true) { failJson(res, 400, 'BROKER_STREAM_REQUIRED', 'Codex OpenAI broker requires stream=true'); return false; }
  if (parsedBody?.store !== false) { failJson(res, 400, 'BROKER_STORE_FORBIDDEN', 'Codex OpenAI broker requires store=false'); return false; }
  if (parsedBody?.background === true) { failJson(res, 400, 'BROKER_BACKGROUND_FORBIDDEN', 'background responses are not allowed'); return false; }
  return true;
}

function validateAnthropicRequestShape(parsedBody, res) {
  if (parsedBody?.stream !== true) {
    failJson(res, 400, 'BROKER_STREAM_REQUIRED', 'Claude Anthropic broker requires stream=true');
    return false;
  }
  if (!Array.isArray(parsedBody?.messages) || parsedBody.messages.length < 1) {
    failJson(res, 400, 'BROKER_MESSAGES_REQUIRED', 'Claude Anthropic broker requires messages');
    return false;
  }
  if (!Number.isSafeInteger(parsedBody?.max_tokens) || parsedBody.max_tokens <= 0 || parsedBody.max_tokens > 64000) {
    failJson(res, 400, 'BROKER_MAX_TOKENS_INVALID', 'Claude max_tokens is outside trusted bounds');
    return false;
  }
  return true;
}

function queryAllowed(url) {
  if (PROVIDER === 'anthropic') return url.search === '' || url.search === '?beta=true';
  return url.search === '';
}
...
async function handleProxy(req, res) {
  if (cleanedUp) { failJson(res, 503, 'BROKER_CLOSED', 'broker has shut down'); return; }
  const capability = presentedCapability(req);
  if (!capability || capability !== CAPABILITY) { failJson(res, 401, 'BROKER_AUTH_INVALID', 'invalid or missing capability'); return; }
  if (Date.now() > capabilityExpiresAt) { failJson(res, 401, 'BROKER_CAPABILITY_EXPIRED', 'capability expired'); return; }
  if (requestCount >= CAPABILITY_MAX_REQUESTS) { failJson(res, 429, 'BROKER_REQUEST_LIMIT', 'request count exhaustion'); return; }
  if (concurrentCount > 0) { failJson(res, 429, 'BROKER_CONCURRENCY_VIOLATION', 'max concurrency 1'); return; }
  if (!upstreamCredential) { failJson(res, 503, 'BROKER_NOT_PROVISIONED', 'no upstream credential'); return; }

  const url = requestUrl(req);
  if (!url || !queryAllowed(url)) {
  ...
  if (body && req.method === 'POST') {
    try { parsedBody = JSON.parse(body); } catch { failJson(res, 400, 'BROKER_MALFORMED_PAYLOAD', 'invalid JSON'); return; }
    if (!parsedBody?.model) { failJson(res, 400, 'BROKER_MODEL_REQUIRED', 'model field is required in request body'); return; }
    const normalized = normalizeModel(parsedBody.model);
    const allowed = normalizeModel(CAPABILITY_MODEL);
    if (normalized !== allowed) { failJson(res, 403, 'BROKER_MODEL_MISMATCH', `model ${parsedBody.model} not allowed`); return; }
    if (PROVIDER === 'openai' && !validateOpenAiRequestShape(parsedBody, res)) return;
    if (PROVIDER === 'anthropic' && !validateAnthropicRequestShape(parsedBody, res)) return;
  }
  ...
  if (PROVIDER === 'anthropic') upstreamHeaders['x-api-key'] = upstreamCredential;
  else upstreamHeaders.Authorization = `Bearer ${upstreamCredential}`;
  if (!upstreamHeaders['Content-Type'] && !upstreamHeaders['content-type']) upstreamHeaders['Content-Type'] = 'application/json';
  if (PROVIDER === 'openai' && openAiProjectId) upstreamHeaders['OpenAI-Project'] = openAiProjectId;

  const upstreamUrl = `${providerBaseUrl()}${url.pathname}${url.search}`;
...
const server = http.createServer((req, res) => {
  const url = requestUrl(req);
  if (url?.pathname === '/health' && !url.search && req.method === 'GET') { handleHealth(req, res); return; }
  handleProxy(req, res);
});
...
async function start() {
  ...
  server.listen(PORT, '0.0.0.0', () => log('info', `provider=${PROVIDER} listening on ${PORT}`));
...
start().catch(async (err) => {
  try { await cleanupProviderCredential(); } catch {}
