...
ordinary_frozen="$(line_of "$ordinary" 'chmod 400 "$patch_file"')"
ordinary_tests="$(line_of "$ordinary" 'run-isolated-repository-tests.sh')"
ordinary_apply="$(line_of "$ordinary" 'apply_agent_patch "$target_dir"')"
...
repair_frozen="$(line_of "$repair" 'chmod 400 "$patch_file"')"
repair_tests="$(line_of "$repair" 'run-isolated-repository-tests.sh')"
repair_apply="$(line_of "$repair" 'apply_agent_patch "$target_dir"')"
t "ordinary agent container finishes before credential scan" "yes" "$([ "$ordinary_container_done" -lt "$ordinary_scan" ] && echo yes || echo no)"
...
t "ordinary patch is frozen before repository tests" "yes" "$([ "$ordinary_frozen" -lt "$ordinary_tests" ] && echo yes || echo no)"
t "ordinary tests finish before trusted patch import" "yes" "$([ "$ordinary_tests" -lt "$ordinary_apply" ] && echo yes || echo no)"
t "repair agent container finishes before credential scan" "yes" "$([ "$repair_container_done" -lt "$repair_scan" ] && echo yes || echo no)"
...
t "repair patch is frozen before repository tests" "yes" "$([ "$repair_frozen" -lt "$repair_tests" ] && echo yes || echo no)"
t "repair tests finish before trusted patch import" "yes" "$([ "$repair_tests" -lt "$repair_apply" ] && echo yes || echo no)"
