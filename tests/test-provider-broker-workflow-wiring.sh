...
t "dispatch passes Admin key only for enabled Codex broker" "yes" \
...
# legacy direct-key mode remains available while the flag is false.
...
t "review repair does not receive OpenAI Admin key" "yes" \
  "$(! grep -Fq 'OPENAI_ADMIN_KEY' "$REPAIR" && echo yes || echo no)"
