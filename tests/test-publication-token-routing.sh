...
# Review Repair: reading/validation can keep GITHUB_TOKEN for same-repo, but
# branch resume git auth and the actual repair push must use the App token so a
# synchronize event can start normal pull_request CI without approval gating.
