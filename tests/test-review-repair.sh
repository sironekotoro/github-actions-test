...
t "untrusted agent patch is checked before host import" "yes" "$(grep -q 'apply_agent_patch' "$CONTAINER_AGENT" && grep -q 'apply --check -p2' "$ROOT/scripts/lib/common.sh" && grep -q 'apply --whitespace=error -p2' "$ROOT/scripts/lib/common.sh" && echo yes || echo no)"
