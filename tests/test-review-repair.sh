...
t "OpenCode auto-approval remains inside hardened outer isolation" "yes" "$(grep -Fq 'opencode run --auto --agent build' "$ROOT/scripts/agents/opencode.sh" && grep -q -- '--network "\$private_network"' "$CONTAINER_AGENT" && grep -q 'src=\$workspace_dir,dst=/workspace' "$CONTAINER_AGENT" && grep -q 'src=\$base_dir,dst=/baseline,readonly' "$CONTAINER_AGENT" && echo yes || echo no)"
...
t "repository tests do not inherit provider credentials" "yes" "$(grep -q 'run-isolated-repository-tests.sh' "$CONTAINER_AGENT" && grep -q 'unset OPENROUTER_API_KEY OPENAI_API_KEY CODEX_API_KEY ANTHROPIC_API_KEY' "$ROOT/scripts/run-isolated-repository-tests.sh" && ! grep -q -- '--env OPENROUTER_API_KEY' "$ROOT/scripts/run-isolated-repository-tests.sh" && echo yes || echo no)"
