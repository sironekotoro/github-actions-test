...
git_cleanup_line="$(grep -n 'rm -rf -- "\$workspace_dir/.git"' "$CONTAINER" | head -n1 | cut -d: -f1)"
credential_scan_line="$(grep -n 'scan_final_workspace_for_credential "\$workspace_dir"' "$CONTAINER" | head -n1 | cut -d: -f1)"
patch_freeze_line="$(grep -n 'patch_file="\$agent_root/agent.patch"' "$CONTAINER" | head -n1 | cut -d: -f1)"
t "agent-created git metadata is discarded before credential scan and patch construction" "yes" "$([ -n "$git_cleanup_line" ] && [ -n "$credential_scan_line" ] && [ -n "$patch_freeze_line" ] && [ "$git_cleanup_line" -lt "$credential_scan_line" ] && [ "$credential_scan_line" -lt "$patch_freeze_line" ] && echo yes || echo no)"
